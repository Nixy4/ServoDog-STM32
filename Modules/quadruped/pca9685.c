...
void pca9685_set_freq(quad_float freq)
{
  ...
  uint8_t psc = ( (uint8_t)( (quad_float)(ICLK) / ( (quad_float)CNT_MAX * freq ) ) ) - 1 ;
  pca9685_set_psc(psc);
}
...
void pca9685_set_angle(uint8_t ledx, quad_float angle)
{
  if (!IS_LEDX(ledx)) {
