...
{
  KinematicsData kdata = {0};
  float K;
  kdata.L7 = sqrt(pow(X, 2) + pow(Z, 2));
  kdata.R17 = acos((pow(L1, 2) + pow(kdata.L7, 2) - pow(L2, 2)) / (2 * L1 * kdata.L7));
  kdata.R12 = acos((pow(L1, 2) + pow(L2, 2) - pow(kdata.L7, 2)) / (2 * L1 * L2));
  if(X==0) {
    kdata.R7X=HPI; 
  } else {
    K= Z / X;
  ...
  kdata.RS1 = kdata.R7X - kdata.R17;
  kdata.R35 = PI - kdata.R12 - R15;
  kdata.L6 = sqrt(pow(L3, 2) + pow(L5, 2) - 2 * L3 * L5 * cos(kdata.R35));
  if(kdata.L6 > L8 + L9) {
    elog_e(TAG, "L6 Error >> X:%f Z:%f", X, Z);
  }
  kdata.RS2 = acos((kdata.L6 * kdata.L6 + L8 * L8 - L9 * L9) / (2 * kdata.L6 * L8));
  kdata.AS1 = DEGREES(kdata.RS1);
  kdata.AS2 = DEGREES(kdata.RS2);
  kdata.R13 = PI - kdata.R12;
  kdata.COORD.X = X;
  kdata.COORD.Z = Z;
  return kdata;
}