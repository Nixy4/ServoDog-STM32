...
static inline double angleMirror(int id, double angle)
{
  if(id == LEG_ID_LF || id == LEG_ID_LB){
    return LA(angle);
  }else{
    return RA(angle);
  }
...
int leg_set_angle(leg_t* leg, double thighAngle, double shankAngle, bool isInverse)
{
  ...
  //角度镜像
  thighAngle = angleMirror(leg->id, thighAngle);
  shankAngle = angleMirror(leg->id, shankAngle);
  //设置舵机角度
  ...
  leg_t* leg, double thighAngle1, double thighAngle2, double shankAngle1, double shankAngle2, double ms)
{
  ...
  //角度镜像
  thighAngle1 = angleMirror(leg->id, thighAngle1);
  thighAngle2 = angleMirror(leg->id, thighAngle2);
  shankAngle1 = angleMirror(leg->id, shankAngle1);
  shankAngle2 = angleMirror(leg->id, shankAngle2);
  //设置舵机角度
...
int leg_move_abselute(leg_t* leg, double x1, double z1, double x2, double z2, double ms)
{
  ...
  if(leg->id==LEG_ID_LF || leg->id==LEG_ID_LB){
